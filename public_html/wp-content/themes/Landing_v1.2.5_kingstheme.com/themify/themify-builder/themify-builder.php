		...
		if( TFCache::check_version()){
			...
			$output = sprintf( '<p><label for="%s"><input type="radio" id="%s" name="%s" value="%s" %s> %s %s</label><br/><label for="%s"><input type="radio" id="%s" name="%s" value="%s" %s> %s %s</label></p>',
					...
					__('(enable it for faster page load)', 'themify')
                        ...
                        $output.='<br/>';
                        $output .= sprintf( '<a href="#" data-clearing-text="%s" data-action="themify_builder_clear_all_caches" data-confim="'.__( 'This will clear all builder caches. click ok to continue.', 'themify' ).'" data-done-text="%s" data-default-text="%s" data-default-icon="ti-eraser" class="button button-outline js-clear-builder-cache"> <i class="ti-eraser"></i> <span>%s</span></a><br><small>%s</small>',
