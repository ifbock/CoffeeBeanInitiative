...
if ( ! function_exists( 'themify_footer_text_left' ) ) {
	...
	function themify_footer_text_left() {
		return '<div class="themify-info-link">' . __( 'Enter your text to replace the copyright and credit links in the footer. HTML tags allowed. Enter an empty space will remove the text.', 'themify' ) . '</div><p><textarea class="widthfull" rows="4" name="setting-footer_text_left">' . esc_textarea( themify_get( 'setting-footer_text_left' ) ) . '</textarea></p>';
	}
}
...
if(!function_exists('themify_manage_script_minification_settings')) {
	...
	function themify_manage_script_minification_settings($data = array()){
		...
		$expire =  themify_get( $key . '_expiry' );
		$expire = $expire>0?intval($expire):2;
		$html = '<p><label for="'. $key.'_disable"><input type="radio" value="disable" id="'. $key.'_disable" name="'. $key.'" '.checked( $value, 'disable', false ).' /> ' . __('Disable minification if you experience frontend issues/conflicts', 'themify') . '</label><br/>';
		$html.='<label for="'. $key.'_enable"><input type="radio" value="enable" id="'. $key.'_enable" name="'. $key.'" '.checked( $value, 'enable', false ).' /> ' . __('Enable minification for faster page load', 'themify') . '</label></p>';
		$html.='<div data-show-if-element="[name='.$key.']:checked" data-show-if-value=' . '["enable"]' . '>';
		$html.=sprintf('<input type="text" class="width2" value="%s" name="%s" />  &nbsp;&nbsp;<span>%s</span>',$expire,$key.'_expiry',__( 'Expire Cache (days)', 'themify' ));
		$html.='<br/><a href="#" data-confim="'.__( 'This will clear all caches. click ok to continue.', 'themify' ).'" data-action="themify_clear_minify_caches" data-clearing-text="'.__('Clearing cache...','themify').'" data-done-text="'.__('Done','themify').'" data-default-text="'.__('Clear cache','themify').'" data-default-icon="ti-eraser" class="button button-outline js-clear-minify-cache js-clear-builder-cache"> <i class="ti-eraser"></i> <span>'.__('Clear minified cache','themify').'</span></a><br/>';
		$html.='<small>'.__('Clear all cache','themify').'</small></div>';
		return $html;
	}
}
